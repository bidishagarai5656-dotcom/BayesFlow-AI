...
def main():
    ...
    model_type = meta.get('modelType', 'gaussian')
    ...
    explanation = []
    explanation.append(f'Predicted "{predicted_class}" with {confidence*100:.1f}% confidence using {model_type.capitalize()} Naive Bayes')
    explanation.append(f'Naive Bayes applies Bayes theorem: P(class|features) ∝ P(class) × P(features|class)')

    sorted_probs = sorted(class_probabilities, key=lambda x: x['probability'], reverse=True)
    if len(sorted_probs) >= 2:
        ratio = sorted_probs[0]['probability'] / max(sorted_probs[1]['probability'], 0.001)
        explanation.append(f'The model is {ratio:.1f}x more confident in "{sorted_probs[0]["className"]}" than "{sorted_probs[1]["className"]}"')
